...
def get_prompts():
    ...
    logging.info(f"Загрузка страницы управления промптами для пользователя: {current_user}")
    ...
    for s in prompts:
        ...
        logging.info(f"Загрузка страницы управления промптами для пользователя: {prompt_info}")
...
def add_prompt():
    ...
    text = data['text']
    try:
        logging.info(f"Добавление нового промпта для пользователя {current_user}: Название: {prompt_name}")
...
def delete_prompt(prompt_id):
    ...
    current_user = get_jwt_identity()
    try:
        logging.info(f"Удаление промпта {prompt_id} для пользователя {current_user}")
        prompt_manager.delete_prompt(prompt_id)
        logging.info(f"Промпт {prompt_id} удален для пользователя {current_user}")
...
def edit_prompt(prompt_id):
    ...
    try:
        logging.info(f"Изменение промпта {prompt_id} для пользователя {current_user}")
        success = prompt_manager.edit_prompt(prompt_id, new_text, new_prompt_name)  # Передаем новое имя

        if success:
            logging.info(f"Промпт {prompt_id} изменен для пользователя {current_user}")
...
def set_automatic(prompt_id):
    ...
    logging.info(f"Запрос на изменение флага 'use_automatic' для промпта {prompt_id} пользователем {current_user}: {use_automatic}")
    try:
        # Сначала сбрасываем флаг для всех остальных промптов, если устанавливаем новый флаг
        if use_automatic:
            logging.info(f"Сброс флага 'use_automatic' для всех промптов пользователя {current_user}.")
        ...
        logging.info(f"Флаг 'use_automatic' для промпта {prompt_id} успешно обновлён на {use_automatic}.")
...
def get_automatic_prompt():
    ...
    try:
        ...
        logging.error(f"Ошибка при получении автоматического промпта: {str(e)}")
